...
// ── Gemini retry helper ──
// Retries once on 429/503/UNAVAILABLE, then falls back to gemini-2.0-flash.
...
app.post('/generate-report', async function(req, res) {
  try {
    ...
    if (!result.ok) {
      console.error('Gemini Error (all retries failed):', JSON.stringify(result.data));
      return res.status(503).json({ error: 'Gemini temporarily unavailable, please try again.', details: result.data });
    }

    console.log('Gemini report (' + result.model + '):', JSON.stringify(result.data).substring(0, 200));
    const report = result.data.choices[0].message.content;
    res.json({ report });
  } catch(error) {
...
- Return ONLY the JSON array, nothing else`;
    ...
    console.log('Gemini OCR (' + result.model + '):', JSON.stringify(data).substring(0, 300));
