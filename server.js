...
app.post('/save-circular', async function(req, res) {
  try {
    const record = {
      ...
      assigned_to:  req.body.assigned_to,     // JS array → Supabase TEXT[]
...
app.post('/circular-summary', async function(req, res) {
  try {
    const { image_base64, image_mime, lang } = req.body;
    if (!image_base64) {
      return res.status(400).json({ error: 'No image provided' });
    }

    const prompt = lang === 'hindi'
      ? 'Yeh ek school circular ki image hai. Is circular ka ek clear aur concise summary SIRF Hindi mein likho. Important points bullet points mein do. Teacher ke liye kya action lena hai woh bhi clearly batao.'
      : 'This is an image of a school circular. Provide a clear and concise summary in English only. Use bullet points for key information. Clearly state what action the teacher needs to take.';
    ...
    const result = await callGeminiNative(
      ['gemini-2.5-flash', 'gemini-2.5-flash-lite'],
      {
        contents: [{
          parts: [
            { inline_data: { mime_type: image_mime || 'image/jpeg', data: image_base64 } },
            { text: prompt }
          ]
        }],
        generationConfig: {
