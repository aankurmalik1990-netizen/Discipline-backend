...
app.post('/read-arrangement', async function(req, res) {
  try {
    ...
    const prompt = `You are reading a handwritten school Teacher's Leave Arrangement sheet.

SHEET STRUCTURE:
- The sheet has a header row with period numbers: 1, 2, 3, 4, 5, 6, 7, 8
- Each COLUMN represents one PERIOD
- Each CELL in a column contains TWO lines:
  1. TOP LINE = CLASS name (like "XII B+A", "X D+C", "IX B", "VIII C+B", etc.)
  2. BOTTOM LINE = TEACHER name of the arrangement teacher (like "Yespal Singh", "Abha Sharma", etc.)
- RULE: Whatever is written ON TOP is the CLASS. Whatever is written BELOW it is the TEACHER.
- One period/column can have MULTIPLE class+teacher pairs stacked vertically — each pair has class on top, teacher below

YOUR TASK: Read every cell in every column and extract each class+teacher pair.

OFFICIAL TEACHER LIST — match every handwritten name to the closest name from this list:
${TEACHER_LIST.join(', ')}

CRITICAL NAME MATCHING RULE:
- Handwriting may be messy or misspelled — use your reasoning to match carefully
- You MUST match each teacher name to the closest name from the official list above
- Examples: "Yespal" → "YESPAL SINGH", "Shubhan" → "SHUBHAM SHARMA", "Ashigeen" → "MH ASHIQEEN", "Shenfali" → "SHAIFALI TOMAR", "R. Pareek" → "ROHITASH PAREEK", "Charoni" → "CHARAN SINGH", "Asligeem" → "MH ASHIQEEN"
- ALWAYS output the exact official name, never the raw handwritten version
...
Rules:
- period: Roman numeral — column 1="I", 2="II", 3="III", 4="IV", 5="V", 6="VI", 7="VII", 8="VIII"
- absentClass: exact class written in cell e.g. "XII B+A", "X D+C", "VIII C+B", "IX A+B"
- mergeClass: null always
- teacher: MUST be exact name from official teacher list
- type: always "duty"
- Extract EVERY cell from EVERY column — do not skip any
