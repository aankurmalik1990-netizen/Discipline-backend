...
// Circular AI Summary using Gemini Vision
app.post('/circular-summary', async function(req, res) {
  try {
    const { pages, image_base64, image_mime, lang } = req.body;

    // Build pages array — support both multi-page and single-page circulars
    ...
    const prompt = lang === 'hindi'
      ? 'Yeh ek school circular ki image hai (ek ya zyada pages). Is circular ka ek detailed aur complete summary SIRF Hindi mein likho. Saare important points bullet points mein do. Koi bhi point miss mat karo. Teacher ke liye kya action lena hai woh bhi clearly batao.'
      : 'This is a school circular image (one or more pages). Provide a detailed and complete summary in English only. Cover ALL key points as bullet points — do not miss anything. Clearly state what action the teacher needs to take.';

    // Build parts — one image part per page + the prompt at the end
    const parts = allPages.map(function(p) {
    ...
    if (!result.ok) {
      console.error('circular-summary Gemini error:', JSON.stringify(result.data));
