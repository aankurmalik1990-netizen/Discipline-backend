...
app.post('/generate-report', async function(req, res) {
  try {
    ...
    if (!geminiRes.ok) {
      console.error('Gemini Report Error:', JSON.stringify(data));
      return res.status(500).json({ error: 'Gemini Error', details: data });
    }
    console.log('Gemini report ok');
...
app.post('/read-arrangement', async function(req, res) {
  try {
    ...
    const prompt = `This is a school Teacher's Leave Arrangement sheet.
...
- absentClass: absent teacher class e.g. "IX A"
- mergeClass: class they merge into (null if duty)
- teacher: arrangement teacher name in CAPS
- type: "merge" or "duty"`;
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(500).json({ error: 'Gemini OCR Error', details: data });
    }

    console.log('Gemini OCR ok:', JSON.stringify(data).substring(0, 200));
