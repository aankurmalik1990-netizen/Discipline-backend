...
Read it carefully and extract ALL arrangement entries.
...
Rules:
- period: Roman numeral (I, II, III, IV, V, VI, VII, VIII)
- absentClass: the class whose teacher is absent (e.g. "IX A", "VIII C")
- mergeClass: the class they are merged INTO (null if duty only)
- teacher: the arrangement/alternate teacher name in CAPS
- type: "merge" if two classes combine, "duty" if teacher just goes to that class
- Extract every single row from the sheet`;
    ...
    const data = await apiResponse.json();
    console.log('Groq Vision status:', apiResponse.status);
    console.log('Groq Vision response:', JSON.stringify(data).substring(0, 500));

    // If model failed, try fallback model
    if (!data.choices || !data.choices[0]) {
      ...
      console.log('Fallback response:', JSON.stringify(fallbackData).substring(0, 300));
      if (!fallbackData.choices || !fallbackData.choices[0]) {
        res.status(500).json({ error: 'AI model error', raw: data });
        return;
      }
    ...
    try {
      ...
      else { res.status(500).json({ error: 'Could not parse AI response', raw: text }); return; }
    }
