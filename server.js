...
app.post('/read-arrangement', async function(req, res) {
  try {
    ...
    // Support both single photo (legacy) and multiple photos array
    const photoList = (images && images.length > 0)
      ? images
      : (imageBase64 ? [{ imageBase64: imageBase64, mimeType: mimeType }] : null);

    if (!photoList) { res.status(400).json({ error: 'No image provided' }); return; }
    ...
    const TEACHER_LIST = [
      ...
      "POOJA KHANNA","ROHIT SAINI","HASAN RAZA"
    ];
...
- Only skip a cell if it is truly blank (nothing written in it). If any text is present, attempt to read it and match to the teacher list.`;
    ...
    // Build parts: all photos first, then the prompt text
    const imageParts = photoList.map(function(p) {
      return { inline_data: { mime_type: p.mimeType || 'image/jpeg', data: p.imageBase64 } };
    });
...
app.post('/circular-summary', async function(req, res) {
  try {
    ...
    const promptHindi = `Yeh ek school circular ki image hai. Poora paragraph summary MAT likho. Sirf neeche diye gaye structured format mein jawab do:

📋 SUMMARY
(1-2 lines mein — yeh circular kis baare mein hai)

📅 DATE
(Circular ki jaari tarikh, agar nahi hai toh "Not mentioned")

⏰ DEADLINE / LAST DATE
(Submission ya compliance ki aakhri tarikh, agar nahi hai toh "Not mentioned")
...
Har section short rakho. Ek section ki baat doosre mein repeat mat karo. Sirf Hindi mein likho.`;
...
⏰ DEADLINE / LAST DATE
(Any submission or compliance deadline. If not mentioned, write "Not mentioned")

✅ ELIGIBLE CRITERION
(Who this applies to — which teachers, staff, classes, or students. If not mentioned, write "All staff")

📌 KEY NOTES
(3-5 bullet points of the most important instructions or highlights only)

👨‍🏫 TEACHER DUTY
(Specific action required from teacher — what they must do, submit, or comply with)

Keep each section short and to the point. Do not repeat information across sections. English only.`;

    const prompt = lang === 'hindi' ? promptHindi : promptEnglish;

    // Build Gemini parts: all images first, then the prompt text
    const circParts = allPages.map(function(p) {
      return { inline_data: { mime_type: p.mime || 'image/jpeg', data: p.base64 } };
    });
    circParts.push({ text: prompt });

    const circResult = await callGeminiNative(
      ['gemini-2.5-flash', 'gemini-2.5-flash-lite'],
      {
    ...
    if (!circResult.ok) {
      console.error('Gemini circular-summary error:', JSON.stringify(circResult.data));
      return res.status(500).json({ error: 'Could not generate summary. Please retry.' });
    }

    const circData = circResult.data;
    if (!circData.candidates[0].content || !circData.candidates[0].content.parts || !circData.candidates[0].content.parts[0]) {
      return res.status(500).json({ error: 'Empty response from AI, please retry.' });
    }

    const summary = circData.candidates[0].content.parts[0].text || '';
...
app.post('/save-arrangement', async function(req, res) {
  try {
    const { date, data, saved_at } = req.body;
    ...
    await fetch(SUPABASE_URL + '/rest/v1/arrangements?date=eq.' + encodeURIComponent(date), {
      method: 'DELETE',
      headers: {
        'Authorization': 'Bearer ' + SUPABASE_KEY,
        'apikey': SUPABASE_KEY
    ...
    await db('arrangements', 'POST', { date: date, data: data, saved_at: saved_at || new Date().toISOString() }, null);
    res.json({ success: true });
  } catch(e) {
...
app.get('/get-arrangement', async function(req, res) {
  try {
    ...
    const rows = await db('arrangements', 'GET', null, 'date=eq.' + encodeURIComponent(date) + '&limit=1');
...
// Delete arrangement for a specific date
