...
app.post('/generate-report', async function(req, res) {
  try {
    ...
    const prompt = `Write a formal school discipline report in Hindi for parents. Student: ${studentName}, Class: ${grade}, Violation: ${incidents[0].violation}, School: ${school}. Keep it under 150 words, formal and respectful.`;
...
Return ONLY a JSON array like this (no explanation, no markdown, no code fences):
[
...
- absentClass: class whose teacher is absent e.g. "IX A", "VIII C"
- mergeClass: class they merge INTO (null if duty only)
- teacher: alternate/arrangement teacher name in CAPS
- type: "merge" if two classes combine, "duty" if teacher goes to that class alone
...
- Return ONLY the JSON array, nothing else`;
    ...
    console.log('Gemini response:', JSON.stringify(geminiData).substring(0, 400));

    if (!geminiData.candidates || !geminiData.candidates[0]) {
      res.status(500).json({ error: 'Gemini response invalid', raw: geminiData });
      return;
    }
    ...
    try {
      ...
      if (match) {
        arrangements = JSON.parse(match[0]);
      } else {
